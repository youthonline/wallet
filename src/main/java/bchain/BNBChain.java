...
public class BNBChain {

    private static BNBChain instance = new BNBChain();

    private BNBChain() {
    }

    public static BNBChain getInstance() {
        return instance;
    }
    ...
    public ReturnsEntity sendTokenTransaction(Web3j web3j, Credentials credentials, String to, String contractAddr, String amount, String gasPrice, String gasLimit) {
        try {
            BigInteger chainId = web3j.ethChainId().send().getChainId();
            // get token decimals
            String tokenDecimals = getDecimals(web3j, credentials.getAddress(), contractAddr);
